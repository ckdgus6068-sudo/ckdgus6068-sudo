...
# (?!들) : '피고소인들이/피의자들' 같은 복수형은 이름이 아니므로 제외
PERSON_RULES = [
    (r"(?:피고소인|피의자)(?!들)\s*\d*\s*[.:：]?\s*(" + NAME + r")", "피의자"),
    (r"(" + NAME + r")\s*[(（]\s*\d{6}\s*-\s*\d{7}", "피의자"),
    (r"고소외(?!인)\s*\d*\s*[:：]?\s*(" + NAME + r")", "고소외"),
...
STOP = {
    ...
    # 복수형/조사 조각(이름 아님)
    "들", "들이", "들은", "들을", "들의", "들도", "들과", "들에", "들에게",
    "등", "등이", "등은", "등을", "고소인들", "피고소인들", "당사자", "관계인",
}
# 회사명 추출용 — 문법어/조사/직함(오탐 방지). _grab 병합을 여기서 멈춘다.
ORG_STOP = {
